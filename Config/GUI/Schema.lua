...
GUI.schema = {
...

	{
		...
		controls = Join({
			...
			{ kind = "check", label = L["Enable Possess Bar"], path = { "ActionBar", "PossessBar", "Enable" }, reload = true, tooltip = L["The small bar that appears while you are controlling another unit, such as a mind control or a vehicle eject."] },
...

	{
		...
		controls = {
			...
			{ kind = "check", label = L["Idle Fade"], path = { "Chat", "IdleFade" }, reload = true, dependsOn = { "Chat", "Enable" }, tooltip = L["Dim the whole chat to a resting alpha when idle and lift it on hover, instead of dropping old lines."] },
			{ kind = "slider", label = L["Idle Fade Alpha"], path = { "Chat", "IdleFadeAlpha" }, min = 0, max = 0.9, step = 0.05, reload = true, dependsOn = { "Chat", "IdleFade" } },
...

	{
		...
		controls = {
			...
			{ kind = "check", label = L["Item Level On Items"], path = { "Tooltip", "ShowItemInfo" }, reload = true, dependsOn = { "Tooltip", "Enable" }, tooltip = L["Add an item level line to gear tooltips. Retail already shows this natively, so it mainly matters on the older flavours."] },
...

	{
		...
		controls = {
			...
			{ kind = "slider", label = L["Trail Spacing"], path = { "Misc", "CursorTrailSpacing" }, min = 4, max = 30, step = 1, reload = true, dependsOn = { "Misc", "CursorTrail" }, tooltip = L["Pixels the pointer must travel before another dot is dropped. Lower is a denser line."] },
			{ kind = "slider", label = L["Trail Lifetime"], path = { "Misc", "CursorTrailLifetime" }, min = 0.15, max = 1.2, step = 0.05, reload = true, dependsOn = { "Misc", "CursorTrail" }, tooltip = L["Seconds a dot takes to fade out. This is what sets how long the tail looks."] },
			{ kind = "dropdown", label = L["Trail Shape"], path = { "Misc", "CursorTrailShape" }, reload = true, dependsOn = { "Misc", "CursorTrail" }, options = {
				{ text = L["Glow"], value = "Glow" },
				{ text = L["Spark"], value = "Spark" },
				{ text = L["Ping"], value = "Ping" },
				{ text = L["Streak"], value = "Streak" },
			} },
			...
			{ kind = "check", label = L["Shrink Over Time"], path = { "Misc", "CursorTrailShrink" }, reload = true, dependsOn = { "Misc", "CursorTrail" }, tooltip = L["Dots get smaller as they fade out."] },
			{ kind = "check", label = L["Trail In Combat Only"], path = { "Misc", "CursorTrailCombat" }, reload = true, dependsOn = { "Misc", "CursorTrail" }, tooltip = L["Show the trail only while you are in combat."] },
			{ kind = "check", label = L["Hide While Turning"], path = { "Misc", "CursorTrailHideTurning" }, reload = true, dependsOn = { "Misc", "CursorTrail" }, tooltip = L["Hide the trail while you turn or look with the mouse, where the pointer is parked and would smear."] },
