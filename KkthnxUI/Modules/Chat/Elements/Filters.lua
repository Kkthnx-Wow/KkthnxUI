...
local firestormBroadcast = {"%-(.*)%|T(.*)|t(.*)|c(.*)%|r", "%[(.*)ARENA ANNOUNCER(.*)%]", "%[(.*)Announce by(.*)%]", "%[(.*)Autobroadcast(.*)%]", "%[(.*)BG Queue Announcer(.*)%]", "has joined the instance group.", "%has joined the instance group.%"}
...
function Module:isFirestormReminder(_, msg, ...)
	for _, filter in ipairs(firestormBroadcast) do
...
function Module:CreateChatFilter()
	...
	-- ChatFrame_AddMessageEventFilter("CHAT_MSG_SYSTEM", self.isFirestormReminder)
	-- ChatFrame_AddMessageEventFilter("CHAT_MSG_RAID_BOSS_EMOTE", self.isFirestormReminder)
	-- ChatFrame_AddMessageEventFilter("CHAT_MSG_CHANNEL", self.isFirestormReminder)
	-- ChatFrame_AddMessageEventFilter("CHAT_MSG_YELL", self.isFirestormReminder)
end