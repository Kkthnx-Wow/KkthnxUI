...
Module.AvadaValueSpells = { -- 显示数值的法术
	[77535] = true, -- 鲜血护盾
}

Module.AvadaGemini = { -- 自动转换的法术
...
function Module:Avada_UpdateWatchData()
	wipe(watchUnits)
	wipe(watchTypes)
...
local function stringParser(str)
	for result in gmatch(str, "[^N]+") do
		...
		local geminiSpell = Module.AvadaGemini[spellID]
...
function Module:Avada_RefreshIcons()
	local specIndex = GetSpecialization()
	if specIndex > 4 then
		specIndex = 1
	end -- use 1st spec for lower level
	...
	local classString = NDuiADB["AvadaProfile"][specID] and NDuiADB["AvadaProfile"][specID][profileIndex] or Module.DefaultAvada[specID]
	if classString then
...
function Module:Avada_RefreshAll()
	Module.Avada_RefreshIcons(self)
	Module:Avada_UpdateWatchData()
end

function Module:Avada_GetUnitAura(unit, spellID, filter)
	for index = 1, 40 do
...
function Module:Avada_UpdateAura(button, unit, spellID, filter)
	local name, count, duration, expire, caster, spellID, value = Module:Avada_GetUnitAura(unit, spellID, filter)
	...
	if avadaValue then
		if Module.AvadaValueSpells[spellID] and value then
			avadaValue:SetText(K.ShortValue(value))
		else
			avadaValue:SetText("")
		end
	end
...
function Module:Avada_UpdateCD(button, spellID)
...
function Module:Avada_UpdateItem(button, itemID)
...
function Module:Avada_OnEvent(event, unit)
	if event == "PLAYER_TARGET_CHANGED" then
		if not watchTypes["buff"] and not watchTypes["debuff"] then
			return
		end
		if not watchUnits["target"] then
			return
		end
		...
		for i = 1, maxButtons do
			...
			if data and data.unit == "target" then
				Module:Avada_UpdateAura(avadaButtons[data.index], data.unit, data.spellID, filter)
			end
		end
	elseif event == "SPELL_UPDATE_COOLDOWN" or event == "SPELL_UPDATE_CHARGES" then
		if not watchTypes["cd"] then
			return
		end
		...
		for i = 1, maxButtons do
			...
			if data and data.type == "cd" then
				Module:Avada_UpdateCD(avadaButtons[data.index], data.spellID)
			end
		end
	elseif event == "BAG_UPDATE_COOLDOWN" then
		if not watchTypes["item"] then
			return
		end
		...
		for i = 1, maxButtons do
			...
			if data and data.type == "item" then
				Module:Avada_UpdateItem(avadaButtons[data.index], data.spellID)
			end
		end
	end
end

function Module:Avada_OnAura(unit)
	if not watchTypes["buff"] and not watchTypes["debuff"] then
		return
	end
	if not watchUnits[unit] then
		return
	end
	...
	for i = 1, maxButtons do
		...
		if data then
			...
			if data.unit == unit and filter then
				Module:Avada_UpdateAura(avadaButtons[data.index], unit, data.spellID, filter)
			end
		end
	end
end

function Module:Avada_Toggle(frame)
	...
	if C.db["Avada"]["Enable"] then
		...
		K:RegisterEvent("UNIT_AURA", Module.Avada_OnAura)
		frame:RegisterEvent("PLAYER_TARGET_CHANGED", Module.Avada_OnEvent, true)
		frame:RegisterEvent("SPELL_UPDATE_COOLDOWN", Module.Avada_OnEvent, true)
		frame:RegisterEvent("SPELL_UPDATE_CHARGES", Module.Avada_OnEvent, true)
		frame:RegisterEvent("BAG_UPDATE_COOLDOWN", Module.Avada_OnEvent, true)

		Module.Avada_RefreshAll(frame)
		frame:RegisterEvent("TRAIT_CONFIG_UPDATED", Module.Avada_RefreshAll, true)
	else
		for i = 1, 6 do
			frame.Avada[i]:Hide()
		end
		K:UnregisterEvent("UNIT_AURA", Module.Avada_OnAura)
		frame:UnregisterEvent("PLAYER_TARGET_CHANGED", Module.Avada_OnEvent)
		frame:UnregisterEvent("SPELL_UPDATE_COOLDOWN", Module.Avada_OnEvent)
		frame:UnregisterEvent("SPELL_UPDATE_CHARGES", Module.Avada_OnEvent)
		frame:UnregisterEvent("BAG_UPDATE_COOLDOWN", Module.Avada_OnEvent)
		frame:UnregisterEvent("TRAIT_CONFIG_UPDATED", Module.Avada_RefreshAll, true)
	end
end

function Module:AvadaKedavra(self)
	local iconSize = (C.db["Nameplate"]["PPWidth"] + 2 * 1 - 6 * (maxButtons - 1)) / maxButtons

	self.Avada = {}
	for i = 1, maxButtons do
		local bu = CreateFrame("Frame", "NDui_AvadaIcon" .. i, self.Health)
		bu:SetSize(iconSize, iconSize)
		-- B.AuraIcon(bu) -- FIX ME
		...
		if i == 1 then
			bu:SetPoint("TOPLEFT", self.Power, "BOTTOMLEFT", -1, -6)
		else
			bu:SetPoint("LEFT", self.Avada[i - 1], "RIGHT", 6, 0)
		end

		self.Avada[i] = bu
	end
	avadaButtons = self.Avada
	Module.avadaData = auraData

	local valueStr = K.CreateFontString(self.Health, 18, "", "", "system")
	valueStr:ClearAllPoints()
	valueStr:SetPoint("RIGHT", self.Health, "LEFT", -5, 0)
	avadaValue = valueStr

	Module:Avada_Toggle(self)
end
