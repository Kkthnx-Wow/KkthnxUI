...
-- Notes:
-- - Purpose: Creates and updates the Raid unit frames.
-- - Design: Features Health, Power, Aura tracking, and Raid-specific indicators.
...
function Module:CreateRaid()
	...
	if C["Raid"].ShowHealPrediction then
		Module:CreateHealPrediction(self, {
			absorbStrips = C["Raid"].AbsorbStrips,
		})
	end
	...
	elseif C["Raid"].RaidBuffsStyle == 1 then
		...
		self.Buffs = CreateFrame("Frame", string_format("%sBuffs", self:GetName()), self.Health)
	...
	if C["Raid"].DispelIcon then
		Module:CreateRaidDispelIcon(self)
	end
