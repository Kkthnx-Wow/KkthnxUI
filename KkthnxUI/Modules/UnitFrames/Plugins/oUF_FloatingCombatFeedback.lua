...
local function Update(self, event, ...)
		...
		if isRightUnit and (unit == "target" and (isPlayer or isPet) or unit == "player") then
			...
			elseif value.suffix == "HEAL" then
				...
				if overhealing > 0 then
					...
					overhealText = format(" (%s)", formatNumber(self, overhealing))
				end
				if amount == 0 and not C["Unitframe"].FCTOverHealing then
					return
				end
				text = format("+%s%s", formatNumber(self, amount), overhealText)
	...
	if text and texture then
		...
		feedbackText:SetFormattedText(element.format, texture, critMark and format("*%s", text) or text)
