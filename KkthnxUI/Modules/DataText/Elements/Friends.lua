...
-- Base Module Hooks
-- ---------------------------------------------------------------------------
local noFriendsStrings = {
	"Guess it's time to talk to the voices in my head.",
	"Forever alone.",
	"Social distancing champion.",
	"NPCs are my real friends anyway.",
	"LF friends, pst.",
	"I'm not crying, I just have pixel dust in my eye.",
	"Solo player mode: Engaged."
}
...
local function onEnter()
	...
	if onlineCount == 0 then
		...
		GameTooltip:AddLine(" ")
		GameTooltip:AddLine(noFriendsStrings[math.random(1, #noFriendsStrings)], 1, 1, 1, 1)
		GameTooltip:Show()
		return
	end
...
function Module:CreateSocialDataText()
	...
	friendsDataText.Text:SetPoint("LEFT", friendsDataText, "LEFT", 18, 0)

	friendsDataText.Texture = friendsDataText:CreateTexture(nil, "ARTWORK")
	friendsDataText.Texture:SetPoint("LEFT", friendsDataText, "LEFT", 0, 1)
	friendsDataText.Texture:SetTexture("Interface\\AddOns\\KkthnxUI\\Media\\DataText\\FriendsIcon")
	friendsDataText.Texture:SetSize(16, 16)
