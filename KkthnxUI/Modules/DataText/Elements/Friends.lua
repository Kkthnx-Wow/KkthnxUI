...
local info = Module:RegisterInfobar("Friends", {"TOPLEFT", UIParent, 105, -4})
...
function info:FriendsPanel_Init()
	...
	infoFrame = CreateFrame("Frame", "KKUI_FriendsFrame", info)
	infoFrame:SetSize(400, 495)
	infoFrame:SetPoint("BOTTOMRIGHT", StoreMicroButton, -15, 30)
	...
	infoFrame:SetScript("OnHide", function()
		if K.EasyMenu:IsShown() then K.EasyMenu:Hide() end
	end)

	K.CreateFontString(infoFrame, 16, "|cff0099ff"..FRIENDS_LIST, "", nil, "TOPLEFT", 15, -10)
	infoFrame.friendCountText = K.CreateFontString(infoFrame, 14, "-/-", "", nil, "TOPRIGHT", -15, -12)
	...
	K.CreateFontString(infoFrame, 13, Module.LineString, "", false, "BOTTOMRIGHT", -12, 42)
	local whspInfo = K.InfoColor..Module.RightButton..L["Whisper"]
	K.CreateFontString(infoFrame, 13, whspInfo, "", false, "BOTTOMRIGHT", -15, 26)
	local invtInfo = K.InfoColor.."ALT +"..Module.LeftButton..L["Invite"]
	K.CreateFontString(infoFrame, 13, invtInfo, "", false, "BOTTOMRIGHT", -15, 10)
end
...
function info:FriendsPanel_CreateButton(parent, index)
	...
	button.name = K.CreateFontString(button, 13, "Tag (name)", "", false, "LEFT", 25, 0)
	button.name:SetPoint("RIGHT", button, "LEFT", 230, 0)
	button.name:SetJustifyH("LEFT")
	button.name:SetTextColor(.5, .7, 1)

	button.zone = K.CreateFontString(button, 13, "Zone", "", false, "RIGHT", -28, 0)
...
info.onEvent = function(self, event, arg1)
	...
	self.text:SetText(format("%s: "..K.MyClassColor.."%d", FRIENDS, info.totalOnline))
...
function Module:CreateSocialDataText()
	...
	Module.FriendsDataTextFrame = CreateFrame("Button", "KKUI_FriendsMircoButton", UIParent)
	Module.FriendsDataTextFrame:SetAllPoints(_G.StoreMicroButton)
	Module.FriendsDataTextFrame:SetSize(_G.StoreMicroButton:GetWidth(), _G.StoreMicroButton:GetHeight())
	Module.FriendsDataTextFrame:SetFrameLevel(_G.StoreMicroButton:GetFrameLevel() + 2)
	...
	Module.FriendsDataTextFrame.Texture2 = Module.FriendsDataTextFrame:CreateTexture(nil, "ARTWORK")
	Module.FriendsDataTextFrame.Texture2:SetTexture(C["Media"].Blank)
	Module.FriendsDataTextFrame.Texture2:SetPoint("BOTTOMLEFT", _G.StoreMicroButton, 2, 2)
	Module.FriendsDataTextFrame.Texture2:SetPoint("BOTTOMRIGHT", _G.StoreMicroButton, -2, 0)
	Module.FriendsDataTextFrame.Texture2:SetVertexColor(135/255, 206/255, 235/255, 0.5)
	...
	Module.FriendsDataTextFrame.Text:SetPoint("CENTER", Module.FriendsDataTextFrame.Texture2, "CENTER", 1, 2)
