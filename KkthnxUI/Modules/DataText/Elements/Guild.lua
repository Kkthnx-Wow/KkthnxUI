...
local info = Module:RegisterInfobar("Guild", {"TOPLEFT", UIParent, 15, -4})
...
function info:GuildPanel_CreateButton(parent, index)
	...
	button.name = K.CreateFontString(button, 13, "Name", "", false, "LEFT", 65, 0)
	button.name:SetPoint("RIGHT", button, "LEFT", 185, 0)
	button.name:SetJustifyH("LEFT")

	button.zone = K.CreateFontString(button, 13, "Zone", "", false, "RIGHT", -2, 0)
...
function info:GuildPanel_Init()
	...
	infoFrame = CreateFrame("Frame", "KKUI_GuildInfobar", UIParent)
	infoFrame:SetSize(335, 495)
	infoFrame:SetPoint("BOTTOMRIGHT", GuildMicroButton, -15, 30)
	...
	gName = K.CreateFontString(infoFrame, 16, "Guild", "", true, "TOPLEFT", 15, -10)
	gOnline = K.CreateFontString(infoFrame, 13, "Online", "", false, "TOPLEFT", 15, -35)
	gApps = K.CreateFontString(infoFrame, 13, "Applications", "", false, "TOPRIGHT", -15, -35)
	gRank = K.CreateFontString(infoFrame, 13, "Rank", "", false, "TOPLEFT", 15, -51)
	...
	K.CreateFontString(bu[1], 13, LEVEL_ABBR, "")
	K.CreateFontString(bu[2], 13, CLASS_ABBR, "")
	K.CreateFontString(bu[3], 13, NAME, "", false, "LEFT", 5, 0)
	K.CreateFontString(bu[4], 13, ZONE, "", false, "RIGHT", -5, 0)

	K.CreateFontString(infoFrame, 13, Module.LineString, "", false, "BOTTOMRIGHT", -12, 58)
	local whspInfo = K.InfoColor..Module.RightButton..L["Whisper"]
	K.CreateFontString(infoFrame, 13, whspInfo, "", false, "BOTTOMRIGHT", -15, 42)
	local invtInfo = K.InfoColor.."ALT +"..Module.LeftButton..L["Invite"]
	K.CreateFontString(infoFrame, 13, invtInfo, "", false, "BOTTOMRIGHT", -15, 26)
	local copyInfo = K.InfoColor.."SHIFT +"..Module.LeftButton..L["Copy Name"]
	K.CreateFontString(infoFrame, 13, copyInfo, "", false, "BOTTOMRIGHT", -15, 10)
...
function info:GuildPanel_Refresh()
	...
	gApps:SetText(format(K.InfoColor..GUILDINFOTAB_APPLICANTS, GetNumGuildApplicants()))
	...
	for i = 1, total do
		...
		if connected or mobile then
			...
			if not info.guildTable[count] then
				info.guildTable[count] = {}
			end
...
info.onEvent = function(self, event, arg1)
	if not IsInGuild() then
		self.text:SetText(GUILD..": "..K.MyClassColor..NONE)
		return
	end
	...
	self.text:SetText(GUILD..": "..K.MyClassColor..online)
