...
local Module = K:NewModule("ExtraQuestButton", "AceHook-3.0", "AceEvent-3.0")
...
function ExtraQuestButton:SetItem(itemLink, texture)
	...
	if (itemLink) then
		...
		if (K.itemBlacklist[itemID]) then
			return
		end
	end
	...
	if (self.itemID) then
		...
		if C["ActionBar"].Enable and C["ActionBar"].Hotkey then K.UpdateHotkey(self) end
...
local function GetClosestQuestItem()
	...
	for questID, questLogIndex in next, worldQuests do
		...
		if (itemLink) then
			...
			if (not areaID) then
				areaID = K.itemAreas[tonumber(strmatch(itemLink, "item:(%d+)"))]
			end
