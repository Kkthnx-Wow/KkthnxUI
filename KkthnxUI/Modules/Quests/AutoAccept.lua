...
-- GLOBALS: next, MINIMAP_TRACKING_TRIVIAL_QUESTS, SelectActiveQuest, SelectGossipActiveQuest
-- GLOBALS: QuestFrame, AcceptQuest, CloseQuest, CompleteQuest, QuestInfoItem_OnClick, QuestInfoRewardsFrame
-- GLOBALS: SelectGossipAvailableQuest, SelectGossipOption, QuestIsFromAreaTrigger, QuestFrame_OnEvent
-- GLOBALS: ShowQuestComplete, C_Timer, UIParent, IsQuestFlaggedCompleted, UseContainerItem
...
local function GetContainerItemQuestLevel(bag, slot)
	...
	for index = 1, questTip:NumLines() do
		local level = tonumber(strmatch(_G["QuickQuestTipTextLeft" .. index]:GetText(), questString))
		if level then
			return level
		end
	end
