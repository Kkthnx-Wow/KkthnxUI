...
function Bags:CreateReagentContainer()
	...
	local Reagent = CreateFrame("Frame", "RavUIReagent", UIParent)
	...
	SwitchBankButton:SetPoint("BOTTOMLEFT", Reagent, "TOPLEFT", 4, 4)
...
function Bags:CreateContainer(storagetype, ...)
	local Container = CreateFrame("Frame", "RavUI".. storagetype, UIParent)
	...
	Container:Hide()
	Container:SetTemplate("Transparent") -- false
	...
	if (storagetype == "Bag") then
		...
		Sort:SetPoint("BOTTOMLEFT", Container, "TOPLEFT", 4, 4)
		...
		ToggleBagsContainer:SetPoint("BOTTOMRIGHT", Container, "TOPRIGHT", -4, 4)
		...
		SwitchReagentButton:SetPoint("BOTTOMLEFT", Container, "TOPLEFT", 4, 4)
		...
		Purchase:SetPoint("BOTTOMLEFT", SwitchReagentButton, "TOPLEFT", -54, 6)
		Purchase:CreateBackdrop("Black")
		...
		for i = 1, 7 do
			...
			Bag:CreateBackdrop()
			Bag:SetBackdropBorderColor(C["Media"].BorderColor[1], C["Media"].BorderColor[2], C["Media"].BorderColor[3])
			Bag:StyleButton()
			Bag:ClearAllPoints()
...
function Bags:SetBagsSearchPosition()
	...
	BagItemSearchBox.Backdrop:SetTemplate("Transparent") -- false
...
function Bags:SkinTokens()
	for i = 1, 3 do
		...
		Token:SetScale(1)
		Token:CreateBackdrop(nil) -- (nil)
		Token.Backdrop:SetOutside(Icon)
		Token.Backdrop:SetBackdropBorderColor(0, 0, 0, 0)
		Token.Backdrop:SetBackdrop(K.BorderBackdrop) --K.BorderBackdrop
		Token.Backdrop:SetBackdropColor(C["Media"].BackdropColor[1], C["Media"].BackdropColor[2], C["Media"].BackdropColor[3], C["Media"].BackdropColor[4])
...
function Bags:SlotUpdate(id, button)
	...
	elseif ItemLink then
		...
		button.Backdrop:SetBackdropBorderColor(C["Media"].BorderColor[1], C["Media"].BorderColor[2], C["Media"].BorderColor[3])
	end
end
