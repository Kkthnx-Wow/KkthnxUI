...
local function ForceUpdateBlizzardFonts()
	...
	if C["Skins"].ReplaceNameFont then
		_G.UNIT_NAME_FONT = NAMEFONT
	end

	if C["Skins"].ReplaceCombatFont then
		_G.DAMAGE_TEXT_FONT = COMBAT
	end

	if C["Skins"].ReplaceBlizzFonts then
		_G.STANDARD_TEXT_FONT	= NORMAL

		local size		= C["Skins"].FontSize
		...
		local s = not C["Skins"].UnifiedBlizzFonts

		SetFont(_G.ChatBubbleFont,						BUBBLE, C["Skins"].ChatBubbleFontSize, C["Skins"].ChatBubbleFontOutline)	-- 13
