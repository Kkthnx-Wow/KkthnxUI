...
function openMail(index)
	if not InboxFrame:IsVisible() then return stopOpening("|cffffff00".."Need a mailbox.") end
	if index == 0 then MiniMapMailFrame:Hide() return stopOpening("|cffffff00".."All done.") end
	...
	if (numItems and numItems > 1) or (items > 1 and index <= items) then
		...
		stopOpening("|cffffff00".."All done.")
...
function onEvent(frame, event, arg1, arg2, arg3, arg4)
	if event == "UI_ERROR_MESSAGE" then
		if arg1 == ERR_INV_FULL then
			stopOpening("|cffffff00".."Stopped, inventory is full.")
		elseif arg1 == ERR_ITEM_MAX_COUNT then
			stopOpening("|cffffff00".."Stopped. Found a duplicate unique item in bag or in bank.")
		end
	end
end
