...
function Module:RaidTool_BuffChecker(parent)
	...
	local BuffName = {"Flask", "Food", SPELL_STAT4_NAME, RAID_BUFF_2, RAID_BUFF_3, RUNES}
	...
	local function sendResult(i)
		...
		if count > 0 then
			if count >= numPlayer then
				sendMsg("Lack of"..BuffName[i]..": "..ALL..PLAYER)
			elseif count >= 5 and i > 2 then
				sendMsg("Lack of"..BuffName[i]..": "..string_format("%s players", count))
			else
				local str = "Lack of"..BuffName[i]..": "
				for j = 1, count do
	...
	local function scanBuff()
		...
		if #NoBuff[1] == 0 and #NoBuff[2] == 0 and #NoBuff[3] == 0 and #NoBuff[4] == 0 and #NoBuff[5] == 0 and #NoBuff[6] == 0 then
			sendMsg("[KkthnxUI] All Buffs Ready!")
		else
			sendMsg("[KkthnxUI] Raid Buff Checker:")
...
function Module:RaidTool_CreateMenu(parent)
	...
	StaticPopupDialogs["Group_Disband"] = {
		...
		OnAccept = function()
			...
			if IsInRaid() then
				SendChatMessage("[KkthnxUI] Raid Disbanding", "RAID")
				for i = 1, GetNumGroupMembers() do
