...
local function LimitLines()
	for i = 1, #ct.frames do
		f = ct.frames[i]
		f:SetMaxLines(f:GetHeight() / C.Media.Combat_Font_Size)
	end
end
	...
	if event == "COMBAT_TEXT_UPDATE" then
		...
		if SHOW_COMBAT_TEXT == "0" then
			...
			if subevent == "DAMAGE" then
				if C.CombatText.ShortNumbers == true then
...
if C.CombatText.DamageStyle then
	DAMAGE_TEXT_FONT = C.Media.Combat_Font
end
...
for i = 1, numf do
	...
	f:SetFont(C.Media.Combat_Font, C.Media.Combat_Font_Size, C.Media.Combat_Font_Style)
	f:SetShadowColor(0, 0, 0, 0, 0)
	f:SetShadowOffset(0, 0, 0, 0)
	...
	f:SetClampRectInsets(0, 0, C.Media.Combat_Font_Size, 0)
	f:SetInsertMode(C.CombatText.Direction or "bottom")
	if i == 1 then
		f:SetJustifyH(ct.justify_1)
		if C.Unitframe.Enable == true and _G.PlayerFrame then
	...
	elseif i == 3 then
		...
		if C.Unitframe.Enable == true and _G.TargetFrame then
			f:SetPoint("BOTTOMRIGHT", "TargetFrame", "TOPRIGHT", 2, 278)
		else
			f:SetPoint("CENTER", 330, 205)
		end
		local a, _, c = f:GetFont()
		if C.Media.Combat_Font_Size == "auto" then
			if C.CombatText.Icons then
				f:SetFont(a, C.CombatText.IconSize / 2, c)
			end
		elseif type(C.Media.Combat_Font_Size) == "number" then
			f:SetFont(a, C.Media.Combat_Font_Size, c)
		end
	end
	ct.frames[i] = f
end
...
if C.CombatText.DeathKnightRunes and K.Class == "DEATHKNIGHT" then
...
local StartConfigmode = function()
	if not InCombatLockdown()then
		for i = 1, #ct.frames do
			...
			f.fs:SetFont(C.Media.Combat_Font, C.Media.Combat_Font_Size, C.Media.Combat_Font_Style)
			...
			if not C.CombatText.Scrollable then
				f:SetScript("OnSizeChanged", function(self)
					self:SetMaxLines(self:GetHeight() / C.Media.Combat_Font_Size)
					self:Clear()
				end)
			end
		...
		if (sourceGUID == ct.pguid and destGUID ~= ct.pguid) or (sourceGUID == UnitGUID("pet") and C.CombatText.PetDamage) or (sourceFlags == gflags) then
			if eventType == "SWING_DAMAGE" then
				...
				if amount >= C.CombatText.Treshold then
					...
					local color = {1, 1, 1}
					if C.CombatText.MergeAoeSpam and C.CombatText.MergeMelee then
						local spellId = 6603
			...
			if eventType == "SPELL_HEAL" or (eventType == "SPELL_PERIODIC_HEAL" and C.CombatText.ShowHots) then
				if C.CombatText.Healing then
					...
					if amount >= C.CombatText.HealTreshold then
						local color = {}
						local rawamount = amount
						if C.CombatText.ShowOverHealing and abs(overhealing) > 0 then
							if C.CombatText.ShortNumbers == true then
