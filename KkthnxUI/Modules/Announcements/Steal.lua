...
function Module:COMBAT_LOG_EVENT_UNFILTERED()
	...
	if C["Announcements"].Steal.Value == "PARTY" then
		SendChatMessage(string_format(StealMessage, destName, spellID, spellName), inPartyLFG and "INSTANCE_CHAT" or "PARTY")
	elseif C["Announcements"].Steal.Value == "RAID" then
		if inRaid then
			SendChatMessage(string_format(StealMessage, destName, spellID, spellName), inPartyLFG and "INSTANCE_CHAT" or "RAID")
		else
			SendChatMessage(string_format(StealMessage, destName, spellID, spellName), inPartyLFG and "INSTANCE_CHAT" or "PARTY")
		end
