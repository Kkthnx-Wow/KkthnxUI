...
local function BuildBAR(barWidth, iconSize)
	...
	frame.Statusbar:SetStatusBarTexture(C["MediaMisc"].Texture)
	...
	frame.Statusbar.Spark:SetTexture(C["MediaMisc"].Spark_16)
...
function Module:AuraWatch_SetupAura(index, UnitID, name, icon, count, duration, expires, id, filter, flash, spellID)
	...
	if frame.glowFrame then
		if flash then
			K.libButtonGlow.ShowOverlayGlow(frame.glowFrame)
		else
			K.libButtonGlow.HideOverlayGlow(frame.glowFrame)
		end
	end
		...
		for _, value in pairs(FrameList) do
			for i = 1, 6 do
				...
				if value[i].glowFrame then
					K.libButtonGlow.HideOverlayGlow(value[i].glowFrame)
				end
			end
