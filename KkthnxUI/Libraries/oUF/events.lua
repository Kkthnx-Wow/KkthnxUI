...
function Private.UpdateUnits(frame, unit, realUnit)
	...
	if(frame.__unit ~= unit or frame.__realUnit ~= realUnit) then
		...
		frame.__unit = unit
		-- KkthnxUI: mirror the resolved unit onto the public field our modules
		-- read (name only plates, auras, tooltip, highlights). Upstream keeps it
		-- private under __unit now, so without this every self.unit read is nil.
		frame.unit = unit
