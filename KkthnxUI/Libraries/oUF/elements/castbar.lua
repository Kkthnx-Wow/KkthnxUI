...
local tradeskillCurrent, tradeskillTotal, mergeTradeskill = 0, 0, false -- KkthnxUI | Sourced: ElvUI
...
local function UNIT_SPELLCAST_START(self, event, unit)
	...
	local name, text, texture, startTime, endTime, isTradeSkill, castID, notInterruptible, spellID = UnitCastingInfo(unit) -- KkthnxUI | Sourced: ElvUI adds isTradeSkill
	if(not name) then
		return element:Hide()
	end
