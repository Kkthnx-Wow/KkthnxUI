...
local function CastStart(self, event, unit)
	...
	-- notInterruptible can be a Midnight secret (retail) or nil (older flavours like
	-- TBC Anniversary, where a tradeskill cast reports no interrupt flag). The setter
	-- handles the secret but rejects nil, so pcall keeps the nil case from throwing.
	if(element.Shield) then pcall(element.Shield.SetAlphaFromBoolean, element.Shield, notInterruptible, 1, 0) end
