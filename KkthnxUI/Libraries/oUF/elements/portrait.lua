...
local function Update(self, event, unit)
	...
	* unit - the unit for which the update has been triggered (string)
	...
	-- include element visibility to avoid needless work when hidden
	local isAvailable = element:IsShown() and UnitIsConnected(unit) and UnitIsVisible(unit)
	local hasStateChanged = event ~= "OnUpdate" or element.guid ~= guid or element.state ~= isAvailable
	if hasStateChanged then
		local isPlayerModel = element.isPlayerModel ~= nil and element.isPlayerModel or element:IsObjectType("PlayerModel")
...
local function Enable(self, unit)
	...
	if element then
		...
		-- cache model type to avoid repeated type checks on frequent updates
		element.isPlayerModel = element:IsObjectType("PlayerModel")
