...
local function Update(frame, event, unit)
	...
	while true do
		...
		if not name then
			...
			icon = icons[key]
			if icon and not K.RaidBuffsIgnore[spellID] and (icon.anyUnit or (caster and icon.fromUnits and icon.fromUnits[caster])) then
...
local function setupIcons(self)
	...
	for _, icon in pairs(icons) do
		...
		if name then
			...
			print("|cffff0000WARNING: spell ID ["..tostring(icon.spellID).."] no longer exists! Report this to Kkthnx.|r")
		end
	end
end
