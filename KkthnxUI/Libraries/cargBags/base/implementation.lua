...
function Implementation:GetItemInfo(bagID, slotID, i)
	...
	if (clink) then
		...
		local texture
		i.name, i.link, i.rarity, i.level, i.minLevel, i.type, i.subType, i.stackCount, i.equipLoc, texture, i.sellPrice, i.classID, i.subClassID, i.bindType = GetItemInfo(clink)
		i.texture = i.texture or texture
		i.rarity = i.rarity or i.quality

		if clink:find("battlepet") then
			local data, name = string_match(clink, "|H(.-)|h(.-)|h")
			local _, _, level, rarity, _, _, _, id = string_match(data, "(%w+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+)")
		...
		elseif clink:find("keystone") then
			local data = string_match(clink, "|H(.-)|h(.-)|h")
			local level = string_match(data, "%w+:%d+:%d+:(%d+)")
