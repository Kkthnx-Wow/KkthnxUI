...
local function InputOnMouseDown(self)
	self:SetAutoFocus(true)
end

local function InputOnEditFocusLost(self)
	self:SetAutoFocus(false)
end

local function InputOnTextChange(self)
	...
	print("InputOnTextChange", self.Group, self.Option, Value)
end

local function InputOnEnterPressed(self)
	self:SetAutoFocus(false)
	self:ClearFocus()
	...
	SetValue(self.Group, self.Option, Value)
	print("InputOnEnterPressed", self.Group, self.Option, Value)
end

local function InputOnMouseWheel(self, delta)
...
local InputWindowOnEnterPressed = function(self)
	...
	if GUI.InputWindow.ActiveInput then
		...
		SetValue(Input.Group, Input.Option, Text) -- ?
		Input.ButtonText:SetText(Text)

		if Input.Hook then
			Input.Hook(Text, Input.Group)
		end
...
function GUI:CreateInputWindow()
	...
	Window.Header:SetSize(300, HeaderHeight)
	Window.Header:SetPoint("TOP", Window, 0, -Spacing)
	Window.Header:CreateBorder()

	Window.Header.Text = Window.Header:CreateFontString(nil, "OVERLAY")
	Window.Header.Text:SetPoint("LEFT", Window.Header, Spacing, 1)
	StyleFont(Window.Header.Text, Font, 12)
	Window.Header.Text:SetJustifyH("LEFT")
	Window.Header.Text:SetText("Input")

	-- Close button
	Window.CloseButton = CreateFrame("Frame", nil, Window)
	Window.CloseButton:SetSize(HeaderHeight, HeaderHeight)
	Window.CloseButton:SetPoint("RIGHT", Window.Header, 0, 0)
	...
	Window.CloseButton:SetScript("OnLeave", function(self)
		self.Texture:SetVertexColor(1, 1, 1)
	end)

	Window.CloseButton.Texture = Window.CloseButton:CreateTexture(nil, "OVERLAY")
	Window.CloseButton.Texture:SetPoint("CENTER", Window.CloseButton, 0, 0)
	Window.CloseButton.Texture:SetSize(20, 20)
	Window.CloseButton.Texture:SetTexture("Interface\\AddOns\\KkthnxUI\\Media\\Textures\\CloseButton_32")

	Window.Inner = CreateFrame("Frame", nil, Window)
	Window.Inner:SetPoint("TOPLEFT", Window.Header, "BOTTOMLEFT", 0, -Spacing)
	Window.Inner:SetPoint("BOTTOMRIGHT", Window, 0, Spacing)
	Window.Inner:CreateBorder()

	Window.Input = CreateFrame("EditBox", nil, Window.Inner)
	StyleFont(Window.Input, Font, 12)
	...
	Window.FadeOut:SetScript("OnFinished", function(self)
		self:GetParent():Hide()
	end)
...
local InputOnEnter = function(self)
	self.Parent.Highlight:SetAlpha(WidgetHighlightAlpha)
end
...
local function CreateInput(self, group, option, text, tooltip, hook)
	local Value = C[group][option]
	...
	Input:SetPoint("LEFT", Anchor, 0, 0)
	Input:SetSize(InputWidth, WidgetHeight)
	Input:CreateBorder()
	Input.Group = group
	Input.Option = option
	Input.Value = Value
	...
	Input.Flash:SetTexture(Texture)
	Input.Flash:SetVertexColor(1, 1, 1, 1)
	...
	Input.Highlight:SetTexture(Texture)
	Input.Highlight:SetVertexColor(123/255, 132/255, 137/255)
	Input.Highlight:SetAlpha(0)

	Input.ButtonText = Input:CreateFontString(nil, "OVERLAY")
	StyleFont(Input.ButtonText, Font, 12)
	Input.ButtonText:SetSize(InputWidth, WidgetHeight)
	...
	Input.Label = Input:CreateFontString(nil, "OVERLAY")
	Input.Label:SetPoint("LEFT", Input, "RIGHT", LabelSpacing, 0)
	Input.Label:SetWidth(WidgetListWidth - (SliderWidth + EditboxWidth) - (Spacing * 5))
	Input.Label:SetJustifyH("LEFT")
	StyleFont(Input.Label, Font, 12)
	Input.Label:SetText(text)
...
GUI.Widgets.CreateInput = CreateInput
...
--local function CreateInputWithButton = function(self, id, value, button, label)
